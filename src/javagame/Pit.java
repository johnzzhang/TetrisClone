...
public class Pit {
	...
	public boolean insertPieceAt(Piece piece, int x, int y) {
		...
		if(isPieceInsertableIn(piece, insertPos)) {
			for(int i = 0; i < 4; i++)
				setBlockAt(i, new Tuple(piece.getPosOfBlock(i).x + insertPos.x, piece.getPosOfBlock(i).y + insertPos.y));
	...
	public boolean isPieceInsertableIn(Piece piece, Tuple pos) {
		...
		for(int idx = 0; idx < 4 && isInsertable; idx++) {
			...
			isInsertable &= (pos.x + blockPos.x >= 0 && pos.x + blockPos.x < width) &&
							...
							(getBlockAt((int)(pos.x + blockPos.x), (int)(pos.y + blockPos.y)) == -1);
		}
	...
	public int getNumberOfLines() { return height; }
	public int getNumberOfColumns() { return width; }
